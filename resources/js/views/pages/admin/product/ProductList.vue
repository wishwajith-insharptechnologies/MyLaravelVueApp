<template>
  ...
  const getProducts = async (updatedPage = null) => {
    ...
    try {
      const { data } = await axiosInstance.get(`/api/projects?page=${pagination.value.current}&per=${pagination.value.pageSize}`);
  ...
  const deletePackage = async (id) => {
    try {
      await axiosInstance.delete(`/api/projects/delete/project/${id}`);
      getProducts();
    } catch (error) {
